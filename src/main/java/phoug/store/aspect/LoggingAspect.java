...
public class LoggingAspect {
    ...
    @Before("execution(* phoug.store.controller.*.*(..))")
    ...
    @AfterReturning(pointcut = "execution(* phoug.store.controller.*.*(..))", returning = "result")
    ...
    // Логирование ошибок
    @AfterThrowing(pointcut = "execution(* phoug.store.controller.*.*(..))", throwing = "error")
    public void logAfterThrowing(JoinPoint joinPoint, Throwable error) {
        if (logger.isErrorEnabled()) { // Проверяем, включен ли уровень ERROR
            logger.error("Error in method: {}."
                    + " Error: {}", joinPoint.getSignature().toShortString(), error.getMessage());
        }
    }
}